...
class PaymentServiceStructureTest {
    private val sourceRoot: Path =
        Path.of("").toAbsolutePath().resolve("src/main/kotlin/com/playsay/payment")
    ...
    fun `root package contains only application launcher`() {
        val misplacedRootFiles = directChildren()
            ...
            .filterNot { path -> path.name == "PaymentServiceApplication.kt" }
            .map { path -> path.name }

        assertTrue(
            misplacedRootFiles.isEmpty(),
            "Only PaymentServiceApplication.kt may live in com.playsay.payment root package: $misplacedRootFiles",
        )
    }
    ...
    fun `rest controllers are isolated in controller package`() {
        ...
        assertTrue(
            ...
            "REST controllers must live in com.playsay.payment.controller: $misplacedControllers",
        )
    }
    ...
    fun `request and response dto classes are isolated in dto package`() {
        ...
        assertTrue(
            ...
            "Request/response DTOs must live in com.playsay.payment.dto: $misplacedDtos",
        )
    }
    ...
    fun `jpa entities are isolated in entity package`() {
        ...
        assertTrue(
            ...
            "JPA entities must live in com.playsay.payment.entity: $misplacedEntities",
        )
    }
    ...
    fun `repositories are isolated in repo package`() {
        ...
        assertTrue(
            ...
            "Spring Data repositories must live in com.playsay.payment.repo: $misplacedRepositories",
        )
    }
    ...
    fun `controllers stay thin`() {
        val controllerViolations = kotlinSources()
            ...
            .filter { source ->
                Regex("""(?m)^data\s+class\s+\w+(Request|Response)\b""").containsMatchIn(source.text) ||
                    ...
                    source.text.contains("PaymentProviderClient") ||
        ...
        assertTrue(
            ...
            "Controllers must not own DTOs, JSON parsing, repositories, provider clients, or low-level HTTP clients: $controllerViolations",
        )
    }
