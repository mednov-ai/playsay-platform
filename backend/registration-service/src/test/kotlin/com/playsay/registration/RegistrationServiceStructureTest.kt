...
class RegistrationServiceStructureTest {
    private val sourceRoot: Path =
        Path.of("").toAbsolutePath().resolve("src/main/kotlin/com/playsay/registration")
    ...
    fun `root package contains only application launcher`() {
        val misplacedRootFiles = directChildren()
            ...
            .filterNot { path -> path.name == "RegistrationServiceApplication.kt" }
            .map { path -> path.name }

        assertTrue(
            misplacedRootFiles.isEmpty(),
            "Only RegistrationServiceApplication.kt may live in com.playsay.registration root package: $misplacedRootFiles",
        )
    }

    @Test
    fun `module uses only approved package folders`() {
        val allowedFolders = setOf("client", "config", "controller", "dto", "entity", "repo", "service", "utils")
    ...
    fun `controllers stay thin`() {
        ...
        assertTrue(
            ...
            "Controllers must not own DTOs, JSON parsing, repositories, or low-level clients: $controllerViolations",
        )
    }
