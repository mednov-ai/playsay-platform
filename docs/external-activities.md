...
6. Pointer, keyboard, drag, and scroll input from unlocked participants uses the collaboration fast lane with LiveKit data as an availability fallback; cursor positions use lossy data at a maximum UI rate of 30 Hz. Wheel deltas are summed into one input packet per animation frame so high-frequency trackpad events do not build a reliable-message backlog.
