...
class MaterialAssetControllerTest : MaterialControllerTestFixture() {
    ...
    fun `teacher uploads reusable image and html game assets without changing material document`() {
        ...
        assertEquals("NOT_NEEDED", gameResponse.body!!.metadata["imageOptimizationStatus"].asText())
        assertEquals("embedded-raster-v1", gameResponse.body!!.metadata["imageOptimizationPolicy"].asText())
        assertEquals(0, gameResponse.body!!.metadata["imageOptimizationBytesSaved"].asInt())
