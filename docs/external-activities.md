...
Version `0.1.8` is the current unpacked/Jenkins artifact. It keeps the version-1 web lifecycle and temporary Chrome Debugger attachment, gives every input an event id and a stable result, deduplicates repeated delivery, and refreshes viewport geometry after resize, reload, back navigation, and top-level navigation. Trusted pointer, wheel, and keyboard input therefore reaches canvas and nested-frame providers such as Wordwall with an observable outcome. Chrome may show its standard debugging notice while sharing; stop/return detaches immediately. Any later shipped extension source, manifest behavior or permissions, runtime bundle, or user-facing asset change must increment the manifest/package patch version again, synchronize the lockfile/tests/install guidance, and confirm the new version on `chrome://extensions`. Chrome Web Store and Edge Add-ons publication are not part of the current release flow.
...
6. Pointer, keyboard, drag, and scroll input from unlocked participants uses the dedicated `playsay-external-activity-v1` collaboration fast lane. If its send/ack is unavailable, the same event id is sent through LiveKit after a bounded delay; the host deduplicates before extension dispatch. Cursor positions remain droppable at a maximum UI rate of 30 Hz, while input and result messages are reliable up to the hard buffer limit. Wheel deltas are summed into one input packet per animation frame so high-frequency trackpad events do not build a reliable-message backlog.

The teacher can lock/unlock student input, navigate back, reload, minimize, or stop the activity. Minimizing is synchronized and retains capture for 60 seconds; reopening resumes the same session. Opening a different activity or ending the retention window tears down tracks, debugger attachment, and the extension-created tab. `STOPPED` and `HOST_IDLE` are ordered and session-scoped: a late idle event from the previous session cannot erase an immediate relaunch, while an authoritative host stop clears a still-unowned `REQUESTED` state created when a student launched before the teacher joined. A named video track from the trusted host is also authoritative evidence of `ACTIVE`, so a student-first request converges even if its separate lifecycle packet is missed; the student explicitly subscribes to a matching published track. Loss of the host video track clears the participant focus even while LiveKit still exposes a stale publication object.
...
- capture starting;
- active sharing;
- a recoverable lifecycle failure;
- a recoverable input-only failure while video and session remain active.

The extension-detection timer runs only until `AWAITING_ACTION`; it is cleared as soon as the extension acknowledges the session. Version 0.1.8 and later include their package version in that acknowledgement. An older or unidentifiable package stops before capture with `EXTENSION_UPDATE_REQUIRED` instead of presenting a false active state with unusable provider input. Teacher lifecycle failures use only the stable codes `FEATURE_UNAVAILABLE`, `EXTENSION_NOT_DETECTED`, `EXTENSION_UPDATE_REQUIRED`, `TARGET_TAB_CLOSED`, `CAPTURE_PERMISSION_DENIED`, `CAPTURE_NOT_SUPPORTED`, `CAPTURE_START_FAILED`, and `EXTENSION_ERROR_UNKNOWN`. Input outcomes use `DISPATCHED`, `BRIDGE_UNAVAILABLE`, `STALE_SESSION`, `INPUT_DISABLED`, `TARGET_UNAVAILABLE`, `VIEWPORT_STALE`, `DEBUGGER_FAILED`, and web-only `ACK_TIMEOUT`. Input failure keeps capture active and shows localized Retry and Return controls plus a safe diagnostics export. Retry closes and cleans the stale attempt, creates a new session id/nonce, and ignores late events from the former session. Students never receive raw Chrome errors, the extension package version, nonce, target tab id, or stream id. An `ACTIVE` session without an attached media track remains covered by the connecting state instead of exposing a black interaction surface.
...
- Successful input is counted only in memory as aggregates. Bounded failure diagnostics may contain only session id, event id, stage, transport, stable result code, and timestamps. Provider URLs/content, pointer coordinates, key values, cookies, credentials, authorization data, tokens, and learner data are prohibited from browser export, metrics labels, and failure logs.

## Manual smoke matrix

Before production promotion, the dev web build contract and complete browser acceptance matrix must pass on `https://dev.online.honey.school/`. After a separately authorized promotion, repeat a bounded canary on both `https://online.honeyschool.ru/` and `https://online.honey.school/`. Record only the displayed extension version, stable status/result codes, lifecycle outcome, and non-sensitive build identity. Exercise the installed `0.1.8` candidate shown on `chrome://extensions`, including student-first launch before teacher connection, provider start, teacher and participant input with a returned result, Return to lesson, and immediate relaunch.
